...
def model_build_func(inputs,kernel_norm, output_channels,var_name, number_of_UNET_backbone_max_pool,using_deep_supervision=True,kernel_initializer=kernel_initializer ):
    ...
    conv3_1 = inception_block(pool2, nb_filter[2], nb_filter[2], dropout_rate = dropout_rate_later,depth_multiplier=False, kernel_norm =kernel_norm)
    ...
    up1_4 = tf.nn.depth_to_space(conv1_1, block_size=2)
    ...
    conv1_4 = Conv2D(nb_filter[1],kernel_size=(3,3), padding='same', strides=1, kernel_initializer=kernel_initializer, kernel_constraint = kernel_norm)(conv1_4)
